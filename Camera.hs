...
-- convert from a tuple of GLdoubles to an OpenGL vector
toVector3 :: Vec3 -> Vector3 GLdouble
toVector3 (a,b,c) =  Vector3 a b c


-- convert from a tuple of GLdoubles to an OpenGL vertex
toVertex3 :: Vec3 -> Vertex3 GLdouble
...
cameraLook camera = do
   let headPos  = vectorAdd (0,0,0) (cpos camera)
   let headView = vectorAdd (0,0,0) (viewPos camera)
...
perpendicular :: Camera -> (GLdouble,GLdouble,GLdouble)
perpendicular cam =
   let
      ...
      newx = x + (sx)
      newz = z + (sz)
   in (vectorSub (newx, y, newz) (cpos cam))
